...
if ( $settings['line_clamp'] ) {
	$style_inline .= 'display:' . '-webkit-box;';
	$style_inline .= '-webkit-line-clamp:' . $settings['line_clamp'] . ';';
	$style_inline .= '-webkit-box-orient:' . 'vertical;';
...
if ( $settings['link'] ) {
	$show_link = true;
	$link_str = '<a aria-label="Heading Link" href="' . esc_url( $settings['link'] ) . '"' . $type_plugin . $rel_attr . '>';
} elseif ( ! empty( $settings['text_source'] ) && isset( $settings['dynamic_content']['source'] ) && 'post' == $settings['dynamic_content']['source'] && isset( $settings['dynamic_content']['post_info'] ) && 'title' == $settings['dynamic_content']['post_info'] ) {
	$show_link = true;
	$link_str = '<a aria-label="Post Title" href="' . esc_url( get_permalink() ) . '"' . $type_plugin . $rel_attr . '>';
} elseif ( ! empty( $settings['text_source'] ) && isset( $settings['dynamic_content']['source'] ) && 'tax' == $settings['dynamic_content']['source'] && isset( $settings['dynamic_content']['tax'] ) && 'title' == $settings['dynamic_content']['tax'] && get_queried_object() && isset( get_queried_object()->term_id ) ) {
	$show_link = true;
	$link_str = '<a aria-label="Taxonomy Title" href="' . esc_url( get_term_link( get_queried_object() ) ) . '"' . $type_plugin . $rel_attr . '>';
} elseif ( ! empty( $settings['text_source'] ) && isset( $settings['dynamic_content']['source'] ) && 'metabox' == $settings['dynamic_content']['source'] && isset( $settings['dynamic_content']['metabox'] ) && 'category_icon' == $settings['dynamic_content']['metabox'] && get_queried_object() && isset( get_queried_object()->term_id ) ) {
	$show_link = true;
	$link_str = '<a aria-label="Product Category" href="' . esc_url( get_term_link( get_queried_object() ) ) . '"' . $type_plugin . $rel_attr . '>';
}

$result = '';

$result .= '<' . esc_html( $settings['tag'] ) . ' class="' . esc_attr( apply_filters( 'porto_elements_wrap_css_class', 'porto-heading' . ( $settings['show_border'] ? ' has-border border-' . esc_attr( $settings['show_border'] ) : '' ) . ( $settings['className'] ? ' ' . trim( $settings['className'] ) : '' ), $atts, 'heading' ) ) . '" style="' . esc_attr( $style_inline ) . '"' . $animation_attrs . ( $show_link ? '' : $type_plugin ) . '>';
