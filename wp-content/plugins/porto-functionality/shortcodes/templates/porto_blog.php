...
if ( $posts->have_posts() ) {
	...
	$wrapper_id = 'porto-blog-' . hash( 'md5',  json_encode( $wrap_arr ) );
	...
	if ( 'modern' == $post_style && 'grid' == $post_layout ) {
		...
		if ( empty( $options ) && function_exists( 'porto_grid_post_column_class' ) ) {
	...
	while ( $posts->have_posts() ) {
		...
		if ( function_exists( 'porto_get_template_part' ) ) {
			...
			porto_get_template_part( 'content', 'blog-' . sanitize_file_name( $template_name ), $args );
			if ( isset( $column_classes ) ) {
				echo '</div>';
			}
		} else {
			get_template_part( 'content', 'blog-' . sanitize_file_name( $template_name ) );
		}
	}
