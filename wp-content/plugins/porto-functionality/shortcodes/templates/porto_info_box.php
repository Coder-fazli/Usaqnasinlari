...
if ( '' == $page_builder ) { // WPBakery Visibility
	$hidden_cls = '';
	$devices = array( 'large-desktop', 'desktop', 'tablet-extra', 'tablet', 'mobile-extra', 'mobile' );
	foreach ( $devices as $device ) {
		if ( ! empty( $atts[$device] ) ) {
			$hidden_cls .= ' ' . 'device-hidden-' . $device;
		}
	}

	if ( $hidden_cls ) {
		$hidden_cls = ' ' . trim( $hidden_cls );
		if ( ! ( $read_more && 'box' == $read_more ) ) {
			$classes .= ' ' . $hidden_cls;
		}
	}
}
...
if ( $link ) {
	if ( 'box' == $read_more ) {
		$html = '<a class="porto-sicon-box-link' . ( ! empty( $hidden_cls ) ? $hidden_cls : '' ) . '" href="' . esc_url( $url ) . '"' . $target . $rel . $link_title . '>' . $html . '</a>';
	}
}
