...
$output = $target = $link = $banner_style_inline = $title_bg = $target = '';
...
$banner_wrapper        = array();
if ( $image_opacity ) {
	$banner_wrapper['image_opacity'] = $image_opacity;
}
if ( $image_opacity_on_hover ) {
	$banner_wrapper['image_opacity_on_hover'] = $image_opacity_on_hover;
}
if ( $box_shadow ) {
	$banner_wrapper['box_shadow'] = $box_shadow;
}
if ( $overlay_color ) {
	$banner_wrapper['overlay_color'] = $overlay_color;
}
if ( $overlay_opacity ) {
	$banner_wrapper['overlay_opacity'] = $overlay_opacity;
}

$interactive_banner_id = 'interactive-banner-wrap-' . hash( 'md5',  json_encode( $banner_wrapper ) );
...
if ( $image_opacity != $image_opacity_on_hover || ( '' != $image_opacity && '1' != $image_opacity ) ) {
	$internal_styles .= '#' . $interactive_banner_id . ' .porto-ibanner-img';
	if ( ! empty( $banner_effect ) && ! empty( $banner_image ) ) {
		$internal_styles .= ',#' . $interactive_banner_id . ' .banner-effect';
	}
	$internal_styles .= '{';
...
if ( $image_opacity != $image_opacity_on_hover ) {
	...
	if ( ! empty( $banner_effect ) && ! empty( $banner_image ) ) {
		$internal_styles .= ',#' . $interactive_banner_id . ':hover .banner-effect';
	}
	$internal_styles .= '{';
...
if ( $href ) {
	$output .= '<a aria-label="Banner & Featured Image" class="porto-ibanner-link" ' . $href . ' ' . $target . ' ' . $link_title . ' ' . $rel . '></a>';
}
$output .= '</div>';
