...
if ( ! function_exists( 'porto_ultimate_heading_spacer' ) ) {
	...
	$custom_class = 'porto-u-' . hash( 'md5',  json_encode( $align ) );
	...
	if ( ! empty( $enable_marquee ) ) {
		...
		$type_plugin .= ' class="marquee" data-duration="' . floatval( $marquee_duration ) . '" data-gap="10" data-duplicated="true" data-startVisible="true" data-clone-element="5"';
	}
	...
	if ( empty( $atts['main_heading_porto_typography'] ) ) {
		if ( ( ! isset( $atts['main_heading_use_theme_fonts'] ) || 'yes' !== $atts['main_heading_use_theme_fonts'] ) && $main_heading_font ) {
			...
			$main_heading_style_inline .= 'font-family:' . urldecode( esc_attr( $main_heading_font_family ) ) . ';';
		}
	...
	if ( empty( $atts['sub_heading_porto_typography'] ) ) {
		if ( ( ! isset( $atts['sub_heading_use_theme_fonts'] ) || 'yes' !== $atts['sub_heading_use_theme_fonts'] ) && $sub_heading_font ) {
			...
			$sub_heading_style_inline .= 'font-family: ' . urldecode( esc_attr( $sub_heading_font_family ) ) . ';';
		}
	}
	...
	} elseif ( ( is_array( $floating_img ) && ! empty( $floating_img['url'] ) ) ) {
		// Elementor External
		$image_info        = array();
		$image_info['src'] = $floating_img['url'];
		if ( ! empty( $floating_img['alt'] ) ) {
			$image_info['alt'] = $floating_img['alt'];
		}
	...
	if ( ! empty( $image_info ) ) {
		...
		if ( ! empty( $image_info['alt'] ) ) {
			$floating_image['alt'] = $image_info['alt'];
		}
		if ( ! empty( $image_info['width'] ) ) {
			$floating_image['width'] = $image_info['width'];
		}
		if ( ! empty( $image_info['height'] ) ) {
			$floating_image['height'] = $image_info['height'];
		}
	...
	if ( defined( 'VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG' ) && isset( $css_porto_ulheading ) ) {
		$el_class .= ' ' . apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css_porto_ulheading, ' ' ), 'porto_ultimate_heading', $atts );
	}
	...
	if ( ! empty( $floating_image ) && ! empty( $image_info ) ) {
		if ( ! empty( $floating_image['height'] ) && ! empty( $floating_image['width'] ) ) {
			$output .= '<span class="thumb-info-floating-element d-none"><img src="' . esc_attr( $floating_image['url'] ) . '" alt="' . ( ! empty( $floating_image['alt'] ) ? esc_attr( $floating_image['alt'] ) : '' ) . '" width="' . esc_attr( $floating_image['width'] ) . '" height="' . esc_attr( $floating_image['height'] ) . '"></span>';
		} else {
			$output .= '<span class="thumb-info-floating-element d-none"><img src="' . esc_attr( $floating_image['url'] ) . '"' . ( ! empty( $floating_image['alt'] ) ? ' alt="' . esc_attr( $floating_image['alt'] ) . '"': '' ) . '></span>';
		}
	}
