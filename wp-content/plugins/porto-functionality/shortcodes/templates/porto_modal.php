...
if ( 'youtube' == $modal_contain || 'vimeo' == $modal_contain ) {
	...
	$modal_array = array();
	if ( ! empty( $atts['modal_wd_sz'] ) ) {
		$modal_array['width'] = $atts['modal_wd_sz'];
	}
	$trigger_id   = 'porto-modal-' . hash( 'md5',  json_encode( $modal_array ) );
	$content_type = 'inline';
}

if ( ! empty( $atts['modal_wd_sz'] ) && ( 'html' == $modal_contain || '' == $modal_contain ) ) {
	$saved_value       = str_replace( '``', '"', $atts['modal_wd_sz'] );
	...
	if ( ! empty( $saved_value ) ) {
		$inline_style = '#' . $trigger_id . '{margin-left: auto; margin-right: auto;width:100%;max-width:' . $saved_value . ( ! empty( $responsive_values['unit'] ) ? $responsive_values['unit'] : 'px' ) . '}';
		$html .= '<style>';
		$html .= $inline_style;
		$html .= '</style>';
	}
