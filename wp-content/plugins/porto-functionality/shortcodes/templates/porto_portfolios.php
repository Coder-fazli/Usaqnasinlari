...
if ( $posts->have_posts() ) {
	...
	if ( $info_color ) {
		$wrap_arr['info_color'] = $info_color;
	}
	if ( $info_color2 ) {
		$wrap_arr['info_color2'] = $info_color2;
	}
	if ( $custom_portfolios ) {
		$wrap_arr['custom_portfolios'] = $custom_portfolios;
	}
	if ( $masonry_layout ) {
		$wrap_arr['masonry_layout'] = $masonry_layout;
	}

	$wrapper_id = 'porto-portfolio-' . hash( 'md5',  json_encode( $wrap_arr ) );
	...
	while ( $posts->have_posts() ) {
		$posts->the_post();
		++$portfolio_num;
		porto_get_template_part( 'content', 'archive-portfolio-' . sanitize_file_name( $portfolio_layout ), $image_size );
	}
