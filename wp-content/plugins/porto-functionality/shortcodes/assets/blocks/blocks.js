		...
		edit: withSelect( function( select, props ) {
			...
			var inspectorControls = el( InspectorControls, {},
				...
				el( ToggleControl, {
					...
					checked: attrs.slider_config ? attrs.slider_config : false,
				...
				attrs.slider_config && el( ToggleControl, {
					...
					checked: attrs.show_nav ? attrs.show_nav : false,
				...
				attrs.slider_config && attrs.show_nav && el( ToggleControl, {
					...
					checked: attrs.show_nav_hover ? attrs.show_nav_hover : false,
					onChange: ( value ) => { props.setAttributes( { show_nav_hover: value } ); },
				} ),
				attrs.slider_config && el( ToggleControl, {
					label: __( 'Show Dots', 'porto-functionality' ),
					checked: attrs.show_dots ? attrs.show_dots : false,
	...
	const PortoCarousel = function( props ) {
		...
		var inspectorControls = el( InspectorControls, {},
			...
			el( ToggleControl, {
				...
				checked: attrs.autoplay ? attrs.autoplay : false,
			...
			attrs.autoplay && el( ToggleControl, {
				...
				checked: attrs.autoplay_hover_pause ? attrs.autoplay_hover_pause : false,
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoUltimateHeading = function( props ) {
		...
		var inspectorControls = el( InspectorControls, {},
			...
			el( PanelBody, {
				...
				el( PortoTypographyControl, {
					...
					options: { textTransform: false, textAlign: false },
					onChange: ( value ) => {
						if ( typeof value.fontFamily != 'undefined' ) {
		...
		if ( attrs.main_heading_letter_spacing ) {
			let unit = attrs.main_heading_letter_spacing.replace( /[0-9.]/g, '' );
			if ( !unit ) {
				attrs.main_heading_letter_spacing += 'px';
			}
		...
		if ( 'line_only' == attrs.spacer ) {
			...
			line_style_inline.float = attrs.alignment;
	...
	registerBlockType( 'porto/porto-ultimate-heading', {
		...
		icon: 'porto',
		category: 'porto',
		keywords: ['title', 'sub title', 'heading separator'],
		description: __( 'Display the heading with description.', 'porto-functionality' ),
		attributes: {
			main_heading: {
				type: 'string',
			},
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoIcons = function( props ) {
		...
		var renderControls = el(
			...
			el( InnerBlocks, { placeholder: ( <h3>{ __( 'Click plus to add', 'porto-functionality' ) }</h3> ), allowedBlocks: ['porto/porto-single-icon'] } ),
		);
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoSingleIcon = function( props ) {
		...
		var inspectorControls = el( Fragment, {}, 
			el( InspectorControls, null,
				el( PanelBody, { title: __( 'General', 'porto-functionality' ) }, 
					el( TextControl, {
						label: __( 'Icon Class (fas fa-user)', 'porto-functionality' ),
						value: attrs.icon,
						onChange: ( value ) => { props.setAttributes( { icon: value } ); },
					} ),
					el(
		...
		var renderControls = el(
			...
			attrs.icon ? el( 'i', { className: attrs.icon } ) : (<i className="fas fa-arrow-right"></i>),
		);
		if ( attrs.icon_link ) {
			renderControls = el(
				'a',
	...
	registerBlockType( 'porto/porto-single-icon', {
		...
		icon: 'porto',
		category: 'porto',
		description: __( 'Add a individual single icon with dynamic link for member or portfolio share links.', 'porto-functionality' ),
		attributes: {
			icon: { type: 'string' },
			icon_size: { type: 'int' },
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoInteractiveBanner = function( props ) {
						...
						jQuery.ajax( {
							...
							success: function( res ) {
								if ( res && res.success ) {
									...
									props.setAttributes( { banner_image: undefined, banner_image_url: undefined } );
								}
							}
						} );
					}
			...
			el( InspectorControls, null,
				...
				el( PanelBody, {
					...
					el(
						...
						!attrs.img_source && attrs.banner_image && el( IconButton, {
							className: 'components-toolbar__control',
							label: __( 'Remove image', 'porto-functionality' ),
							icon: 'no',
							onClick: function onClick() {
						...
						!attrs.img_source && attrs.banner_image && el( 'img', {
							src: attrs.banner_image_url,
							className: 'components-upload-show-img'
						} ),
					),
					attrs.banner_image && el( ToggleControl, {
						label: __( 'Lazy Load Image', 'porto-functionality' ),
						checked: attrs.lazyload ? attrs.lazyload : false,
						onChange: ( value ) => { props.setAttributes( { lazyload: value } ); },
					...
					el( ToggleControl, {
						...
						checked: attrs.add_container ? attrs.add_container : false,
						onChange: ( value ) => { props.setAttributes( { add_container: value } ); },
					} ),
					el( TextControl, {
						label: __( 'Min Height (e.g: 450px)', 'porto-functionality' ),
					...
					( attrs.img_source == 'dynamic' || attrs.banner_image ) && el( SelectControl, {
		...
		var renderControls = el(
			...
			( attrs.banner_effect || attrs.particle_effect ) && attrs.banner_image_url && el(
				'div',
			...
			attrs.banner_image_url && el(
				'img',
				{ className: 'porto-ibanner-img', src: attrs.banner_image_url, style: img_style }
			),
			el(
				'div',
	...
	registerBlockType( 'porto/porto-interactive-banner', {
		title: 'Dynamic Image with Content',
		icon: 'porto',
		category: 'porto',
		description: __( 'Show the content with image including dynamic image like taxonomy, product or post image.', 'porto-functionality' ),
		keywords: ['featured image', 'dynamic image', 'acf image', 'toolset', 'author avatar', 'category image', 'product image', 'interactive banner'],
		attributes: {
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoInteractiveBannerLayer = function( props ) {
		...
		if ( 50 == Number( attrs.horizontal ) ) {
			extra_style.left = '50%';
			extra_style.right = 'unset';
			extra_style.transform = 'translateX(-50%)';
			if ( 50 == Number( attrs.vertical ) ) {
				extra_style.left = '50%';
				extra_style.top = '50%';
				extra_style.right = 'unset';
				extra_style.bottom = 'unset';
				extra_style.transform = 'translate(-50%, -50%)';
			}
		...
		if ( 50 == Number( attrs.vertical ) ) {
			if ( 50 != Number( attrs.horizontal ) ) {
				extra_style.transform = 'translateY(-50%)';
				extra_style.top = '50%';
				extra_style.bottom = 'unset';
			}
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
	...
	const PortoHeading = function( props ) {
		...
		var inspectorControls = el( Fragment, {},
			el( InspectorControls, {},
				el( PanelBody, { title: __( 'General', 'porto-functionality' ) },
					...
					el( SelectControl, {
						label: __( 'Line Clamp', 'porto-functionality' ),
						value: attrs.line_clamp,
						options: [{ label: __( 'Select', 'porto-functionality' ), value: '' }, { label: __( '1', 'porto-functionality' ), value: '1' }, { label: __( '2', 'porto-functionality' ), value: '2' }, { label: __( '3', 'porto-functionality' ), value: '3' }, { label: __( '4', 'porto-functionality' ), value: '4' }, { label: __( '5', 'porto-functionality' ), value: '5' }, { label: __( '6', 'porto-functionality' ), value: '6' }, { label: __( '7', 'porto-functionality' ), value: '7' }, { label: __( '8', 'porto-functionality' ), value: '8' }],
						onChange: ( value ) => { props.setAttributes( { line_clamp: value } ); },
					} ),
					el( TextControl, {
						label: __( 'Link', 'porto-functionality' ),
					...
					el( ToggleControl, {
						label: __( 'Add nofollow', 'porto-functionality' ),
						checked: attrs.nofollow ? attrs.nofollow : false,
				...
				el( PanelBody, { title: __( 'TypeWriter Effect', 'porto-functionality' ), initialOpen: false },
					el( ToggleControl, {
						...
						checked: attrs.enable_typewriter ? attrs.enable_typewriter : false,
		...
		if ( attrs.line_clamp ) {
			style_inline['display'] = '-webkit-box';
			style_inline['WebkitLineClamp'] = attrs.line_clamp;
			style_inline['WebkitBoxOrient'] = 'vertical';
			style_inline['overflow'] = 'hidden';
		}
		...
		var renderControls = el(
			...
			( attrs.text_source && ! isLink ) ? el(
				...
				wp.element.RawHTML({ children: realHeadingText })
			)
			: (	isLink ? el(
					attrs.tag,
					{
	...
	const withCustomClassName = createHigherOrderComponent( ( BlockListBlock ) => {
		return ( props ) => {
			if( props.attributes.is_block ) {
				return <BlockListBlock key={ 'b-' + props.clientId } { ...props } className={ 'porto-gb-btn-block' } />;
			} else {
				return <BlockListBlock key={ 'b-' + props.clientId } { ...props } />
			}
			...
			el( InspectorControls, {},
				el( PanelBody, { title: __( 'Layout', 'porto-functionality' ) }, 
					...
					el( ToggleControl, {
						...
						checked: attrs.is_block ? attrs.is_block : false,
...
( function( wpI18n, wpBlocks, wpElement, wpEditor, wpBlockEditor, wpComponents, wpData, lodash ) {
			...
			el( InspectorControls, {},
				el( PanelBody, { title: __( 'General', 'porto-functionality' ) }, 
					...
					el( ToggleControl, {
						...
						checked: attrs.flex_container ? attrs.flex_container : false,
					...
					attrs.flex_container && el( ToggleControl, {
						...
						checked: attrs.flex_wrap ? attrs.flex_wrap : false,
				...
				el( PanelBody, { title: __( 'Shape Divider', 'porto-functionality' ), initialOpen: false }, 
					...
					attrs.top_bottom == 'top' && attrs.top_divider_type != 'none' && el( ToggleControl, {
						...
						checked: attrs.top_divider_flip ? attrs.top_divider_flip : false,
						onChange: ( value ) => { props.setAttributes( { top_divider_flip: value } ); }
					} ),
					attrs.top_bottom == 'top' && attrs.top_divider_type != 'none' && el( ToggleControl, {
						label: __( 'Top Divider Invert', 'porto-functionality' ),
						checked: attrs.top_divider_invert ? attrs.top_divider_invert : false,
					...
					attrs.top_bottom == 'bottom' && attrs.bottom_divider_type != 'none' && el( ToggleControl, {
						...
						checked: attrs.bottom_divider_flip ? attrs.bottom_divider_flip : false,
						onChange: ( value ) => { props.setAttributes( { bottom_divider_flip: value } ); }
					} ),
					attrs.top_bottom == 'bottom' && attrs.bottom_divider_type != 'none' && el( ToggleControl, {
						label: __( 'Bottom Divider Invert', 'porto-functionality' ),
						checked: attrs.bottom_divider_invert ? attrs.bottom_divider_invert : false,
	...
	registerBlockType( 'porto/porto-section', {
		...
		icon: 'porto',
		category: 'porto',
		description: __( 'Place content elements with shape divider, section separator, flexbox container.', 'porto-functionality' ),
			...
			<InspectorControls>
				<PanelBody title={ __( 'Countdown Timer', 'porto-functionality' ) } initialOpen={ true }>
					...
					<ToggleControl
						...
						checked={ attrs.enable_dynamic_date ? attrs.enable_dynamic_date : false }
