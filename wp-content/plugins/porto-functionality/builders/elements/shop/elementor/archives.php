...
class Porto_Elementor_SB_Archives_Widget extends \Elementor\Widget_Base {
	...
	public function get_keywords() {
		return array( 'posts', 'shop products', 'product archive', 'type', 'post card', 'archives', 'builder', 'custom', 'shop', 'post' , 'product category' );
	}

	public function get_icon() {
		return 'eicon-posts-group porto-elementor-widget-icon';
	}
	...
	public function get_script_depends() {
		if ( isset( $_REQUEST['elementor-preview'] ) ) {
			if ( ! wp_style_is( 'jquery-hover3d', 'registered' ) ) {
				wp_register_script( 'jquery-hover3d', PORTO_SHORTCODES_URL . 'assets/js/jquery.hover3d.min.js', array(), PORTO_FUNC_VERSION, true );
			}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'description' => sprintf( __( 'Please leave empty if you want to use default value which is set using WooCommerce -> Product Archives -> Products per Page in %1$sTheme Options%2$s.', 'porto-functionality' ), '<a target="_blank" class="porto-text-underline" href="' . porto_get_theme_option_url( 'category-item' ) . '">', '</a>' ),
				'range'       => array(
					'px' => array(
						'step' => 1,
						'min'  => 1,
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					''          => __( 'Default (No Ajax)', 'porto-functionality' ),
					'ajax'      => __( 'Ajax Pagination', 'porto-functionality' ),
					'infinite'  => __( 'Infinite Scroll (Ajax)', 'porto-functionality' ),
					'load_more' => __( 'Load more (Ajax)', 'porto-functionality' ),
	...
	protected function render() {
		...
		if ( class_exists( 'Woocommerce' ) && ( $template = porto_shortcode_template( 'porto_posts_grid' ) ) ) {
			...
			if ( is_singular( PortoBuilders::BUILDER_SLUG ) || ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'elementor_ajax' == $_REQUEST['action'] && ( ! empty( $_REQUEST['editor_post_id'] ) || ! empty( $_REQUEST['initial_document_id'] ) ) ) ) {
