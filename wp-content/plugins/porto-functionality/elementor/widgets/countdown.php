...
class Porto_Elementor_Countdown_Widget extends \Elementor\Widget_Base {
	...
	public function get_keywords() {
		return array( 'date', 'timer', 'sale countdown', 'WooCommerce', 'product', 'flash sale end date' );
	}

	public function get_icon() {
		return 'eicon-countdown porto-elementor-widget-icon';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'description' => __( 'Enable option to show the sale countdown on single product page.', 'porto-functionality' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'multiple' => true,
				'default'  => array( 'sday', 'shr', 'smin', 'ssec' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'description' => __( 'This label is shown if timer works.', 'porto-functionality' ),
				'condition'   => array(
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'selectors'   => array(
						'{{WRAPPER}} .porto_countdown' => 'display: flex; flex-wrap: wrap;',
						'.elementor-element-{{ID}} .porto_countdown' => 'justify-content: {{VALUE}}',
					),
				)
			);
		$this->end_controls_section();
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						...
						'.elementor-element-{{ID}} .porto-cd-s2 .porto_countdown-section .porto_countdown-amount' => 'text-align: {{VALUE}}',
					),
				)
			);
			...
			$this->add_control(
				...
				array(
					'type'        => Controls_Manager::SLIDER,
					'label'       => __( 'Label Space(px)', 'porto-functionality' ),
					'range'       => array(
						'px' => array(
							'step' => 1,
							'min'  => 1,
					...
					'qa_selector' => '.countdown-label',
					'selectors'   => array(
						'.elementor-element-{{ID}} .countdown-label' => "margin-{$right}: {{SIZE}}px;",
					),
				)
			);
	...
	protected function content_template() {
		...
		<# if ( settings.countdown_label && 'porto-cd-s1' == settings.count_style ) { #>	
			<span class="countdown-label"><# print( settings.countdown_label ) #></span>
		<# } #>	
		<# if ( settings.datetime ) { #>
