...
jQuery( document ).ready( function( $ ) {
	...
	if ( typeof elementorFrontend != 'undefined' ) {
		var porto_elementor_init = function() {
			...
			theme.innerHeight = window.innerHeight;
			/**
			 * Quick Access to Main Header in Header Template, Porto Section( Column ) Additional Option
			 * 
			 * @since 2.10.0
			 */
			$( document.body ).on('click', '.elementor-section .porto-el-section-shortcode, .elementor-column .porto-el-column-shortcode, .e-con .porto-el-container-shortcode', function ( e ) {
				var $this = $( this );
				...
				if ( $this.hasClass( 'porto-el-container-shortcode' ) ) {
					elementor.getPanelView().currentPageView.activateSection( 'section_container_additional' );
					...
					if ( !_modal.length ) {
						...
						_modal.click( function( e ) {
							...
							if ( elementor.selection.getElements()[0] && elementor.selection.getElements()[0].model ) {
								...
								if ( curModel.attributes.widgetType && ['porto_posts_grid', 'porto_cp_linked', 'porto_sb_archives', 'porto_archive_posts_grid', 'porto_single_related', 'porto_archive_category'].indexOf( curModel.attributes.widgetType ) > -1 ) {
			...
			if ( typeof porto_init == 'function' ) {
				...
				elementorFrontend.hooks.addAction( 'frontend/element_ready/section', function( $obj ) {
					...
					// Stretch section with css
					$obj.removeClass( 'porto_stretch_css stretch_row_css stretch_row_content_css' );
					if ( $obj.find( '>[data-stretch-cls]' ).length ) {
						$obj.removeClass( 'stretch_row_css stretch_row_content_css' );
						var $wrapper = $obj.find( '>[data-stretch-cls]' );
						$obj.addClass( $wrapper.data( 'stretch-cls' ) );
						$obj.addClass( 'porto_stretch_css' );
					}
				...
				elementorFrontend.hooks.addAction( 'frontend/element_ready/container', function( $obj ) {
					...
					if ( $obj.children( '.content-collapse-empty' ).length ) {
						$obj.addClass( 'content-collapse-wrap' );
					} else {
						$obj.removeClass( 'content-collapse-wrap' );
					}

					if ( $obj.children( '.porto-parallax' ).length ) {
						var $el = $obj.children( '.porto-parallax' ),
							speed = $el.data( 'parallax-speed' ),
							parallaxType = '';
							
						if ( $el.attr( 'data-parallax-type' ) ) {
							$obj.attr( 'data-parallax-type', 'horizontal' );
							parallaxType = 'horizontal';
						} else {
							$obj.removeAttr( 'data-parallax-type' );
						...
						var opts = { speed: speed },
							parallaxScale = $el.attr( 'data-parallax-scale' ),
							instance = $obj.data( '__parallax' );
						if ( typeof parallaxScale !== 'undefined' ) {
							opts['scale'] = true;
							if ( parallaxScale == 'invert' ) {
					...
					if ( $obj.children( '.shop-loop-before' ).length ) {
						$obj.addClass( 'shop-loop-before' );
					}

					$obj.removeClass( 'porto_stretch_css stretch_row_css stretch_row_content_css' );
					if ( $obj.children( '.porto_stretch_css' ).length ) {
						var $wrapper = $obj.children( '.porto_stretch_css' );
						$obj.addClass( $wrapper.attr( 'class' ) );
					}
				} );
			...
			function initWidgetAddon( $obj ) {
				...
				// Focus to Slider Item
				if ( 'yes' == widgetData['enable_focus'] && widgetData['slider_selector'] && widgetData['item_order'] ) {
					$obj.addClass( 'porto-focus-slider' );
					$obj.attr( 'data-focus-slider', JSON.stringify( { 'selector': widgetData['slider_selector'], 'order': widgetData['item_order'] - 1 } ) );
				} else {
					$obj.removeClass( 'porto-focus-slider' );
					$obj.removeAttr( 'data-focus-slider' );
				}
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_button.default', function( $obj ) {
				if ( $obj.find( '.btn-read-more-action' ).length ) {
					$obj.addClass( 'btn-read-more-wrap' );
				} else {
					$obj.removeClass( 'btn-read-more-wrap' );
				}
			} );
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_sidebar_menu.default', function( $obj ) {
				...
				}, 950 ); // After refresh the side header width
			} );
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_archive_category.default', function( $obj ) {
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_ultimate_heading.default', function( $obj ) {
				...
				// Ultimate Heading
				if ( $obj.find( '[data-clone-element]' ).length && ! $obj.find( '.js-marquee' ).length ) {
					$obj.find( '[data-clone-element]' ).each( function() {
						var $el = $(this)
						  , content = $el.html()
				...
				if ( $obj.find( '.marquee' ).length && ! $obj.find( '.js-marquee' ).length && $.isFunction( $.fn.marquee ) ) {
					$obj.find( '.marquee' ).marquee( {
						duration: 5000,
						gap: 0,
						delayBeforeStart: 0,
						direction: 'left',
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_hb_menu.default', function( $obj ) {
				...
				if ( $( '.porto-popup-menu' ).length ) {
					$( '.porto-popup-menu .hamburguer-btn' ).off( 'click' ).on( 'click', function( e ) {
						e.preventDefault();
						var $this = $( this );
					...
					$( '.porto-popup-menu' ).off( 'click' ).on( 'click', 'li.menu-item-has-children > a', function( e ) {
						e.preventDefault();
			...
			elementorFrontend.hooks.addAction( 'frontend/element_ready/porto_hb_toggle.default', function( $obj ) {
				// menu
				if ( typeof theme.MegaMenu !== 'undefined' ) {
					theme.MegaMenu.defaults.menu = $( '.mega-menu' );
					theme.MegaMenu.initialize();
			...
			elementorFrontend.hooks.addAction( 'refresh_edit_area', function( width ) {
				...
				if ( '' == width ) {
					...
					$style.html( '#elementor-add-new-section, .elementor-edit-area > .elementor-section-wrap { max-width: ' + parseFloat( js_porto_vars.container_width ) + 'px; margin: 0 auto; } #elementor-add-new-section .elementor-add-section-inner, .elementor-section-wrap > .elementor-section, .elementor-section-wrap > .e-con { max-width: ' + width + '; }' );
				}
			} );

			elementorFrontend.hooks.addAction( 'refresh_side_header', function( width ) {
				var $style = $( 'style#porto-side-header-style' );
				if ( !$style.length ) {
					$( '.elementor-edit-area' ).before( '<style id="porto-side-header-style"></style' );
					$style = $( 'style#porto-side-header-style' );
				}
				if ( '' == width ) {
					width = 255;
				}
				width += 'px';
				$style.html( '.elementor-add-section-inner{ position: relative; } .elementor-select-preset-list .elementor-preset { width: 33.3333%} .porto-el-container-shortcodes, .porto-el-section-shortcode { display: none; } #elementor-add-new-section, .elementor-edit-area > .elementor-section-wrap { max-width: ' + parseFloat( js_porto_vars.container_width ) + 'px; margin: 0 auto; }#elementor-add-new-section .elementor-add-section-inner, .elementor-section-wrap > .elementor-section, .elementor-section-wrap > .e-con { max-width: ' + width + '; }' );
				
			} );

			elementorFrontend.hooks.addAction( 'refresh_header_type', function() {
				var $style = $( 'style#porto-side-header-style' );
				if ( $style.length ) {
					$style.html( '' );
				}
			...
			['porto_sidebar_menu.default', 'porto_hb_menu.default', 'shortcode.default', 'sidebar.default', 'wp-widget-block-widget.default'].forEach( function( element_name ) {
				elementorFrontend.hooks.addAction( 'frontend/element_ready/' + element_name, function( $obj ) {
			...
			if ( typeof elementorFrontend.elementsHandler.elementsHandlers.container[3] == 'function' ) {
				elementorFrontend.elementsHandler.elementsHandlers.container[3]().then(result => {
					result.default.prototype.onElementChange = function( propertyName ) {
						if ( propertyName.match( /^shape_divider_(top|bottom)_custom$/ ) ) {
							this.buildSVG( propertyName.match( /^shape_divider_(top|bottom)_custom$/ )[1] );
							return;
						}
					...
					result.default.prototype.buildSVG = function buildSVG( side ) {
						var baseSettingKey = 'shape_divider_' + side,
							shapeType = this.getElementSettings( baseSettingKey ),
							$svgContainer = this.elements['$' + side + 'Container'];
						$svgContainer.attr( 'data-shape', shapeType );
